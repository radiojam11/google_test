...
def solution(n,b):
    ...
    while True :
        ...
        #calcolo  il valore della z in base 10 
        if b != 10:
            x=int(x, base=b)
            y=int(y, base=b)
            z = x - y
            stringaNum =""               #costruiso Z in base ai valori del resto della divisione, cifra per cifra, per ricavere Z in base b
            while z>0:  
                stringaNum += str(z%b)   # qui ottengo i numeri che formano Z in senso contrario
                z=z//b
            
            z = ""
            for i in range (len(stringaNum)-1, 0, -1):   # qui metto i numeri di Z nel verso giusto 
                z += stringaNum[i]
        else:
            z=int(x)-int(y)
            z = str(z)
